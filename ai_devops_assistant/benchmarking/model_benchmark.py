...
class AdvancedModelBenchmark:
    ...
    async def _single_run(
        ...
        try:
            ...
            response = await asyncio.wait_for(model_fn(prompt), timeout=self.timeout_seconds)
    ...
    async def compare_models(
        ...
        benchmark_tasks = [
            self.benchmark_model(name, fn, prompts, runs_per_prompt) for name, fn in models.items()
        ]
    ...
    def _generate_comparison(self, results: list[ModelBenchmarkResult]) -> BenchmarkComparison:
        ...
        if not results:
            return BenchmarkComparison(
                models=[], best_performer="", ranking=[], recommendations=[], summary_stats={}
            )
        ...
        for result in results:
            score = (
                ...
                + (  # 40% weight on reliability
                    1.0 - (result.avg_latency_ms / max(r.avg_latency_ms for r in results))
                )
                * 0.3
                + (  # 30% on speed (inverted)
                    result.tokens_per_second / max(r.tokens_per_second or 1 for r in results)
                )
                * 0.3  # 30% on throughput
            )
    ...
    def _generate_recommendations(self, ranking: list[dict[str, Any]]) -> list[str]:
        ...
        recommendations.append(
            f"🏆 Top performer: {best_model} (composite score: {ranking[0]['composite_score']})"
        )
        ...
        if low_reliability:
            recommendations.append(
                f"⚠️  Low reliability detected for: {', '.join(r['model'] for r in low_reliability)}"
            )
        ...
        if len(ranking) > 1:
            ...
            if speedup > 2:
                recommendations.append(
                    f"⚡ {fastest['model']} is {speedup:.1f}x faster than {slowest['model']}"
                )
        ...
        if high_throughput:
            recommendations.append(
                f"🚀 High throughput models: {', '.join(r['model'] for r in high_throughput)}"
            )
    ...
    def load_result(self, file_path: str) -> ModelBenchmarkResult:
        ...
        runs = [BenchmarkMetrics(**r) for r in data["runs"]]

        return ModelBenchmarkResult(**{k: v for k, v in data.items() if k != "runs"}, runs=runs)

    def generate_report(
        self, results: list[ModelBenchmarkResult], output_file: str = "benchmark_report.md"
    ) -> None:
